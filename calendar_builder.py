...
def sp(y,h,o,home=True,c='Premier League',uid_y=None):
 d=datetime.strptime(y+h,'%Y%m%d%H:%M').replace(tzinfo=UK).astimezone(timezone.utc); slug=o.lower().replace('&','and').replace(' ','-').replace("'",'').replace('.',''); uy=uid_y or y; timed(f'spurs-{uy[:4]}-{uy[4:6]}-{uy[6:]}-{slug}@jackallege17-sports',f"⚽ Tottenham — {'vs' if home else 'at'} {o} ({c})",d.strftime('%Y%m%dT%H%MZ'),135)
